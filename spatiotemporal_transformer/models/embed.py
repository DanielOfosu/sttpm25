...
class Embedding(nn.Module):
    def __init__(
        ...
        self.time_emb = utils.time2vec.Time2Vec(d_x, embed_dim=time_dim)

        self.max_seq_len = max_seq_len
        self.position_emb = position_emb
        if self.position_emb == "t2v":
            # standard periodic pos emb but w/ learnable coeffs
            self.local_emb = utils.time2vec.Time2Vec(1, embed_dim=d_model + 1)

        y_emb_inp_dim = d_y if self.method == "temporal" else 1
        self.val_time_emb = nn.Linear(y_emb_inp_dim + time_dim, d_model).bfloat16()
    ...
    def temporal_embed(self, y: torch.Tensor, x: torch.Tensor):
        ...
        if self.position_emb == "t2v":
            ...
            local_emb = self.local_emb(
                local_pos.view(1, -1, 1).repeat(bs, 1, 1)
            )[:, :, 1:]
        ...
        val_time_inp = torch.cat((time_emb, y), dim=-1).bfloat16()
        val_time_emb = self.val_time_emb(val_time_inp).bfloat16()
    ...
    def spatio_temporal_embed(self, y: torch.Tensor, x: torch.Tensor):
        ...
        if self.is_encoder:
            for conv in self.downsize_convs:
                val_time_emb = conv(val_time_emb).bfloat16()
                length //= 2
