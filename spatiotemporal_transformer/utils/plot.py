"Function for plotting the attention matrices, credits to https://github.com/QData/spacetimeformer"
...
from utils.forecasting_metrics import mape
...
class AttentionMatrixCallback(pl.Callback):
    ...
    def _make_imgs(self, attns, img_title_prefix):
        ...
        for head in heads:
            ...
            imgs.append(
                wandb.Image(
                    show_image(
                        a_head.float().cpu().numpy(), # cast to float from fp16 or bf16
    ...
    def on_validation_end(self, trainer, model):
        ...
        enc_emb_sim = self._pos_sim_scores(
            model.model.enc_embedding, # TODO: FIX weird bug
            seq_len=self.test_data[1].shape[1],
            device=model.device,
        )
        dec_emb_sim = self._pos_sim_scores(
            model.model.dec_embedding, # TODO: FIX
